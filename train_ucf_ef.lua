...
local LSTM = require 'model.LSTM_f'
...
if string.len(opt.init_from) > 0 then
    ...
    protos = {}
    protos.rnn = LSTM.lstm(input_size, output_size, opt.rnn_size, opt.num_layers, opt.dropout)
...
function loaddata()
    ...
    local f1_v1 = io.open("/home/arjun/public_speaking/trainfiles01.txt","r")
    local f1_lb = io.open("/home/arjun/public_speaking/trainlabels01.txt","r")
    ...
    while true do
        ...
        local data_m1 = matio.load('/data/arjun/ucf_features/motion/' .. filename)
        local data_m2 = matio.load('/data/arjun/ucf_features/appearance/' .. filename)
    ...
    local f2_v1 = io.open("/home/arjun/public_speaking/testfiles01.txt","r")
    local f2_lb = io.open("/home/arjun/public_speaking/testlabels01.txt","r")
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local val_loss, val_acc, val_spec, val_sen= eval_split(2) -- 2 = validation
        val_losses[i] = val_loss
        val_accs[i] = val_acc

        local savefile = string.format('%s/lm_%s_%d_%d_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, opt.rnn_size, opt.num_layers, epoch, val_acc)
