...
local LSTM4 = require 'model.LSTM4'
...
cmd:option('-reduced_size', 192, 'reduced dimensionality of combination from video and audio LSTMS')
...
cmd:option('-init_from_vid', '', 'initialize visual network parameters from checkpoint at this path')
cmd:option('-init_from_aud', '', 'initialize audio network parameters from checkpoint at this path')
-- bookkeeping
...
if string.len(opt.init_from_vid) > 0 then
    print('loading a visual LSTM from checkpoint ' .. opt.init_from_vid)
    local checkpoint_vid = torch.load(opt.init_from_vid)
    protos_old_vid = checkpoint_vid.protos
    ...
    print('overwriting rnn_size_vid=' .. checkpoint_vid.opt.rnn_size .. ', num_layers_vid=' .. checkpoint_vid.opt.num_layers .. ' based on the checkpoint.')
    opt.rnn_size_vid = checkpoint_vid.opt.rnn_size
    opt.num_layers_vid = checkpoint_vid.opt.num_layers
    --do_random_init = false

    old_weights_vid = {}
    old_biases_vid = {}
    for layer_idx = 1, opt.num_layers_vid do
        for _,node in ipairs(protos_old_vid.rnn.forwardnodes) do
            if node.data.annotations.name == "i2h_" .. layer_idx then
                print('saving i2h weights and biases for layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                old_weights_vid[2*layer_idx-1] = node.data.module.weight
                old_biases_vid[2*layer_idx-1] = node.data.module.bias
            end
            if node.data.annotations.name == "h2h_" .. layer_idx then
                print('saving h2h weights and biases for layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                old_weights_vid[2*layer_idx] = node.data.module.weight
                old_biases_vid[2*layer_idx] = node.data.module.bias
            end
        end
    end
end

if string.len(opt.init_from_aud) > 0 then
    print('loading an audio LSTM from checkpoint ' .. opt.init_from_aud)
    local checkpoint_aud = torch.load(opt.init_from_aud)
    protos_old_aud = checkpoint_aud.protos
    ---- make sure the vocabs are the same
    --local vocab_compatible = true
    --for c,i in pairs(checkpoint.vocab) do
    --    if not vocab[c] == i then
    ...
    --assert(vocab_compatible, 'error, the character vocabulary for this dataset and the one in the saved checkpoint are not the same. This is trouble.')
    -- overwrite model settings based on checkpoint to ensure compatibility
    print('overwriting rnn_size_aud=' .. checkpoint_aud.opt.rnn_size .. ', num_layers_aud=' .. checkpoint_aud.opt.num_layers .. ' based on the checkpoint.')
    opt.rnn_size_aud = checkpoint_aud.opt.rnn_size
    opt.num_layers_aud = checkpoint_aud.opt.num_layers
    --do_random_init = false

    old_weights_aud = {}
    old_biases_aud = {}
    for layer_idx = 1, opt.num_layers_aud do
        for _,node in ipairs(protos_old_aud.rnn.forwardnodes) do
            if node.data.annotations.name == "i2h_" .. layer_idx then
                print('saving i2h weights and biases for layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                old_weights_aud[2*layer_idx-1] = node.data.module.weight
                old_biases_aud[2*layer_idx-1] = node.data.module.bias
            end
            if node.data.annotations.name == "h2h_" .. layer_idx then
                print('saving h2h weights and biases for layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                old_weights_aud[2*layer_idx] = node.data.module.weight
                old_biases_aud[2*layer_idx] = node.data.module.bias
            end
        end
    end
end

    print('creating an LSTM4 with ' .. opt.num_layers_vid .. ' layers for video, ' .. opt.num_layers_aud .. ' layers for audio and ' .. opt.num_layers_comb .. ' layers for combination')
    protos = {}
    protos.rnn = LSTM4.lstm(input_size_vid, input_size_aud, output_size, opt.rnn_size_vid, opt.rnn_size_aud, opt.rnn_size_comb, opt.num_layers_vid, opt.num_layers_aud, opt.num_layers_comb, opt.reduced_size, opt.dropout)
...
function storageInSet(set, storage)
...
function flatten(net_params,flag)
    ...
    for k = 1,#parameters do
        ...
        parameters[k]:set(flatStorage,storageOffset + parameters[k]:storageOffset(),parameters[k]:size(),parameters[k]:stride())
        parameters[k]:zero()
    end
    ...
    for k = 1,#parameters do
        ...
        parameters[k]:set(flatUsedStorage,parameters[k]:storageOffset() - offset,parameters[k]:size(),parameters[k]:stride())
    end
...
tab_of_params, tab_of_grad_params = protos.rnn:parameters()
--print(tab_of_params)
--print(tab_of_grad_params)
params = flatten(tab_of_params,1)
grad_params = flatten(tab_of_grad_params,1)

flat_grad_params = {}
flat_params = {}
for n_mods = 1, #tab_of_grad_params do
    flat_grad_params[n_mods] = tab_of_grad_params[n_mods]:view(tab_of_grad_params[n_mods]:nElement())
    flat_params[n_mods] = tab_of_params[n_mods]:view(tab_of_params[n_mods]:nElement())
end
...
if do_random_init then
    ...
    if opt.model == 'lstm' then
        for layer_idx = 1, math.max(opt.num_layers_vid,opt.num_layers_aud) do
            for _,node in ipairs(protos.rnn.forwardnodes) do
                if node.data.annotations.name == "i2h_v_" .. layer_idx then
                    print('setting i2h weights and biases to pre-trained values for video layer ' .. layer_idx)
                    -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                    node.data.module.bias:copy(old_biases_vid[2*layer_idx-1])
                    node.data.module.weight:copy(old_weights_vid[2*layer_idx-1])
                end

                if node.data.annotations.name == "i2h_a_" .. layer_idx then
                    print('setting i2h weights and biases to pre-trained values for audio layer ' .. layer_idx)
                    -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                    node.data.module.bias:copy(old_biases_aud[2*layer_idx-1])
                    node.data.module.weight:copy(old_weights_aud[2*layer_idx-1])
                    
                end
                if node.data.annotations.name == "h2h_v_" .. layer_idx then
                    print('setting h2h weights and biases to pre-trained values for video layer ' .. layer_idx)
                    -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                    node.data.module.bias:copy(old_biases_vid[2*layer_idx])
                    node.data.module.weight:copy(old_weights_vid[2*layer_idx])
                end

                if node.data.annotations.name == "h2h_a_" .. layer_idx then
                    print('setting h2h weights and biases to pre-trained values for audio layer ' .. layer_idx)
                    -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                    node.data.module.bias:copy(old_biases_aud[2*layer_idx])
                    node.data.module.weight:copy(old_weights_aud[2*layer_idx])
                end
            end
        end
    end
...
for i = 1, iterations do
    ...
    feval_loss, _ = feval(params)

    for n_mods = 1, #tab_of_params do

        if tab_of_params[n_mods]:nDimension() ~= 0 then
            local layerOptimConfig = {}
            local feval2 = function(x)
                return feval_loss, flat_grad_params[n_mods]
            end
            if n_mods > #tab_of_params - 8 then
                layerOptimConfig.learningRate = opt.learning_rate
            else
                layerOptimConfig.learningRate = 0
            end
            --layerOptimConfig.learningRates = torch.zeros(flat_grad_params[n_mods]:size(1), 1):fill(opt.learning_rate)
            layerOptimConfig.alpha = opt.decay_rate
            _, loss = optim.rmsprop(feval2, flat_params[n_mods],layerOptimConfig)
        end
    end
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/lm_%s_%d_%d_%d_%d_%d_%d_%d_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, opt.rnn_size_vid, opt.rnn_size_aud, opt.rnn_size_comb, opt.reduced_size, opt.num_layers_vid, opt.num_layers_aud, opt.num_layers_comb, epoch, val_acc)
