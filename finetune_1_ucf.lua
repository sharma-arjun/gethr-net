...
local LSTM1 = require 'model.LSTM1'
...
if string.len(opt.init_from_aud) > 0 then
    ...
    print('creating an LSTM1 with ' .. opt.num_layers_vid .. ' layers for video and ' .. opt.num_layers_aud .. ' for audio')
    protos = {}
    protos.rnn = LSTM1.lstm(input_size_vid, input_size_aud, output_size, opt.rnn_size_vid, opt.rnn_size_aud, opt.num_layers_vid, opt.num_layers_aud, opt.dropout)
...
for i = 1, iterations do
    ...
    for n_mods = 1, #tab_of_params do
        ...
        if tab_of_params[n_mods]:nDimension() ~= 0 then
            ...
            if n_mods > #tab_of_params - 2 then 
                layerOptimConfig.learningRate = opt.learning_rate
            else
                layerOptimConfig.learningRate = opt.learning_rate/10
            end
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/lm_%s_%d_%d_%d_%d_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, opt.rnn_size_vid, opt.rnn_size_aud, opt.num_layers_vid, opt.num_layers_aud, epoch, val_acc)
