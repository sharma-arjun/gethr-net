...
local LSTM = require 'model.LSTM_f_2dl_dr'
...
-- model params
cmd:option('-rnn_size_1', 128, 'size of LSTM internal state of type 1')
cmd:option('-num_layers_1', 2, 'number of layers in the LSTM of type 1')
cmd:option('-rnn_size_2', 128, 'size of LSTM internal state of type 2')
cmd:option('-num_layers_2', 2, 'number of layers in the LSTM of type 2')
cmd:option('-reduced_size', 1024,'reduced dimensionality of input')
...
if string.len(opt.init_from) > 0 then
    ...
    print('creating an LSTM with ' .. opt.num_layers_1 .. ' layers of type 1 and ' .. opt.num_layers_2 .. ' layers of type 2')
    protos = {}
    protos.rnn = LSTM.lstm(input_size, output_size, opt.reduced_size, opt.rnn_size_1, opt.num_layers_1, opt.rnn_size_2, opt.num_layers_2, opt.dropout)
...
init_state = {}
for L=1,opt.num_layers_1 do
    local h_init = torch.zeros(opt.batch_size, opt.rnn_size_1)
    if opt.gpuid >=0 and opt.opencl == 0 then h_init = h_init:cuda() end
    if opt.gpuid >=0 and opt.opencl == 1 then h_init = h_init:cl() end
    table.insert(init_state, h_init:clone())
    table.insert(init_state, h_init:clone())
end
for L=1,opt.num_layers_2 do
    local h_init = torch.zeros(opt.batch_size, opt.rnn_size_2)
    if opt.gpuid >=0 and opt.opencl == 0 then h_init = h_init:cuda() end
    if opt.gpuid >=0 and opt.opencl == 1 then h_init = h_init:cl() end
    table.insert(init_state, h_init:clone())
    table.insert(init_state, h_init:clone())
...
if opt.model == 'lstm' then
    for layer_idx = 1, opt.num_layers_1+opt.num_layers_2 do
        for _,node in ipairs(protos.rnn.forwardnodes) do
            if node.data.annotations.name == "i2h_1_" .. layer_idx then
                print('setting forget gate biases to 1 in LSTM layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                node.data.module.bias[{{opt.rnn_size_1+1, 2*opt.rnn_size_1}}]:fill(1.0)
            end
            if node.data.annotations.name == "i2h_2_" .. layer_idx then
                print('setting forget gate biases to 1 in LSTM layer ' .. layer_idx)
                -- the gates are, in order, i,f,o,g, so f is the 2nd block of weights
                node.data.module.bias[{{opt.rnn_size_2+1, 2*opt.rnn_size_2}}]:fill(1.0)
            end
        end
    end
end
...
function loaddata()
    ...
    local f1_1 = io.open("/home/arjun/public_speaking/trainfiles0" .. tostring(split_no) .. ".txt","r")
    local f1_2 = io.open("/home/arjun/public_speaking/trainlabels0" .. tostring(split_no) .. ".txt","r")
    ...
    while true do
        local filename = f1_1:read("*line")
        if filename == nil then break end

        local data = matio.load('/data/arjun/ucf_features/split' .. tostring(split_no) .. '/' .. modality .. '/' .. filename)
        local raw_feat = data['features']
        raw_feat = raw_feat:t()
        local label = f1_2:read("*number")
        n_frames = raw_feat:size(1)/10
        local feat = torch.Tensor(n_frames, input_size):zero()
        for i=1,n_frames*10 do
            feat[{(i%n_frames)+1,{}}] = feat[{(i%n_frames)+1,{}}] + raw_feat[{i,{}}]
        end
        feat:div(10)
    ...
    local f2_1 = io.open("/home/arjun/public_speaking/testfiles0" .. tostring(split_no) .. ".txt","r")
    local f2_2 = io.open("/home/arjun/public_speaking/testlabels0" .. tostring(split_no) .. ".txt","r")
    ...
    while true do
        filename = f2_1:read("*line")
        if filename == nil then break end

        local data = matio.load('/data/arjun/ucf_features/split' .. tostring(split_no) .. '/' .. modality .. '/' .. filename)
        raw_val_feat = data['features']
        raw_val_feat = raw_val_feat:t()
        local label = f2_2:read("*number")
        n_frames = raw_val_feat:size(1)/10
        local val_feat = torch.Tensor(n_frames,input_size):zero()
        for i=1,n_frames*10 do
            val_feat[{(i%n_frames)+1,{}}] = val_feat[{(i%n_frames)+1,{}}] + raw_val_feat[{i,{}}]
        end
        val_feat:div(10)
...
function feval(p)
    ...
    for t=1,n_frames do
        ...
        inp = torch.CudaTensor(1,input_size):zero()
        inp[{1,{}}] = x[{t,{}}]
        --print(t)
        --print(inp)
        local lst = clones.rnn[t]:forward{inp, unpack(rnn_state[t-1])}
    ...
    for t=n_frames,1,-1 do
        ...
        inp = torch.CudaTensor(1,input_size):zero()
        inp[{1,{}}] = x[{t,{}}]
        local dlst = clones.rnn[t]:backward({inp, unpack(rnn_state[t-1])}, drnn_state[t])
        drnn_state[t-1] = {}
...
local optim_state = {learningRate = opt.learning_rate, alpha = opt.decay_rate} -- optim_state for rmsprop
--local optim_state = {learningRate = opt.learning_rate, momentum = 0.9, weightDecay = 0.0005} -- optim_state for nag
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/lm_%s_%d_%d_%d_%d_%d_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, opt.rnn_size_1, opt.num_layers_1, opt.rnn_size_2, opt.num_layers_2, opt.reduced_size, epoch, val_acc)
