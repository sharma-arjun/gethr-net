...
local LSTM = require 'model.LSTM_f_dr'
...
-- model params
cmd:option('-rnn_size', 128, 'size of LSTM internal state for video')
cmd:option('-num_layers', 2, 'number of layers in the LSTM for audio')
cmd:option('-reduced_size', 1024, 'reduced input dimensionality')
...
if string.len(opt.init_from) > 0 then
    ...
    protos = {}
    protos.rnn = LSTM.lstm(input_size, opt.reduced_size, output_size, opt.rnn_size, opt.num_layers, opt.dropout)
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/lm_%s_%d_%d_%d_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, opt.rnn_size, opt.num_layers, opt.reduced_size, epoch, val_acc)
